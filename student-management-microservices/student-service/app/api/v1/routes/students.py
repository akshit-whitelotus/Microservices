...
def update_student(student_id: int,student: StudentUpdate,db: Session = Depends(get_db),_: TokenPayload = Depends(get_current_user),):
    return service.update_student(db=db,student_id=student_id,student_update=student,)
@router.delete("/{student_id}",status_code=status.HTTP_204_NO_CONTENT,)
def delete_student(student_id: int,db: Session = Depends(get_db),_: TokenPayload = Depends(get_current_user),):
